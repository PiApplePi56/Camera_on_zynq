...
localparam
    ...
    ADDR_BAYER_PHASE_DATA_0 = 6'h28,
    ADDR_BAYER_PHASE_CTRL   = 6'h2c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BAYER_PHASE_DATA_0: begin
                    rdata <= int_bayer_phase[15:0];
                end
            endcase
        end
    end
...
assign bayer_phase       = int_bayer_phase;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
// int_bayer_phase[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_bayer_phase[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BAYER_PHASE_DATA_0)
            int_bayer_phase[15:0] <= (WDATA[31:0] & wmask) | (int_bayer_phase[15:0] & ~wmask);
    end
end
