//Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
//Copyright 2022-2023 Advanced Micro Devices, Inc. All Rights Reserved.
//--------------------------------------------------------------------------------
//Tool Version: Vivado v.2023.1 (win64) Build 3865809 Sun May  7 15:05:29 MDT 2023
//Date        : Mon Jan  6 15:29:28 2025
//Host        : Pie6 running 64-bit major release  (build 9200)
//Command     : generate_target design_1_wrapper.bd
//Design      : design_1_wrapper
//Purpose     : IP block netlist
//--------------------------------------------------------------------------------
`timescale 1 ps / 1 ps

module design_1_wrapper
   (DDR_addr,
    DDR_ba,
    DDR_cas_n,
    DDR_ck_n,
    DDR_ck_p,
    DDR_cke,
    DDR_cs_n,
    DDR_dm,
    DDR_dq,
    DDR_dqs_n,
    DDR_dqs_p,
    DDR_odt,
    DDR_ras_n,
    DDR_reset_n,
    DDR_we_n,
    FIXED_IO_ddr_vrn,
    FIXED_IO_ddr_vrp,
    FIXED_IO_mio,
    FIXED_IO_ps_clk,
    FIXED_IO_ps_porb,
    FIXED_IO_ps_srstb,
    IIC_BUS_scl_io,
    IIC_BUS_sda_io,
    IIC_CAM_scl_io,
    IIC_CAM_sda_io,
    IIC_HDMI_scl_io,
    IIC_HDMI_sda_io,
    aud_adcdat,
    aud_adclrc,
    aud_bclk,
    aud_dacdat,
    aud_daclrc,
    aud_mclk,
    gpio0_tri_io,
    pl_led_tri_o,
    tmds_tmds_clk_n,
    tmds_tmds_clk_p,
    tmds_tmds_data_n,
    tmds_tmds_data_p);
  inout [14:0]DDR_addr;
  inout [2:0]DDR_ba;
  inout DDR_cas_n;
  inout DDR_ck_n;
  inout DDR_ck_p;
  inout DDR_cke;
  inout DDR_cs_n;
  inout [3:0]DDR_dm;
  inout [31:0]DDR_dq;
  inout [3:0]DDR_dqs_n;
  inout [3:0]DDR_dqs_p;
  inout DDR_odt;
  inout DDR_ras_n;
  inout DDR_reset_n;
  inout DDR_we_n;
  inout FIXED_IO_ddr_vrn;
  inout FIXED_IO_ddr_vrp;
  inout [53:0]FIXED_IO_mio;
  inout FIXED_IO_ps_clk;
  inout FIXED_IO_ps_porb;
  inout FIXED_IO_ps_srstb;
  inout IIC_BUS_scl_io;
  inout IIC_BUS_sda_io;
  inout IIC_CAM_scl_io;
  inout IIC_CAM_sda_io;
  inout IIC_HDMI_scl_io;
  inout IIC_HDMI_sda_io;
  input aud_adcdat;
  output aud_adclrc;
  output [0:0]aud_bclk;
  output aud_dacdat;
  output aud_daclrc;
  output aud_mclk;
  inout [9:0]gpio0_tri_io;
  output [2:0]pl_led_tri_o;
  output tmds_tmds_clk_n;
  output tmds_tmds_clk_p;
  output [2:0]tmds_tmds_data_n;
  output [2:0]tmds_tmds_data_p;

  wire [14:0]DDR_addr;
  wire [2:0]DDR_ba;
  wire DDR_cas_n;
  wire DDR_ck_n;
  wire DDR_ck_p;
  wire DDR_cke;
  wire DDR_cs_n;
  wire [3:0]DDR_dm;
  wire [31:0]DDR_dq;
  wire [3:0]DDR_dqs_n;
  wire [3:0]DDR_dqs_p;
  wire DDR_odt;
  wire DDR_ras_n;
  wire DDR_reset_n;
  wire DDR_we_n;
  wire FIXED_IO_ddr_vrn;
  wire FIXED_IO_ddr_vrp;
  wire [53:0]FIXED_IO_mio;
  wire FIXED_IO_ps_clk;
  wire FIXED_IO_ps_porb;
  wire FIXED_IO_ps_srstb;
  wire IIC_BUS_scl_i;
  wire IIC_BUS_scl_io;
  wire IIC_BUS_scl_o;
  wire IIC_BUS_scl_t;
  wire IIC_BUS_sda_i;
  wire IIC_BUS_sda_io;
  wire IIC_BUS_sda_o;
  wire IIC_BUS_sda_t;
  wire IIC_CAM_scl_i;
  wire IIC_CAM_scl_io;
  wire IIC_CAM_scl_o;
  wire IIC_CAM_scl_t;
  wire IIC_CAM_sda_i;
  wire IIC_CAM_sda_io;
  wire IIC_CAM_sda_o;
  wire IIC_CAM_sda_t;
  wire IIC_HDMI_scl_i;
  wire IIC_HDMI_scl_io;
  wire IIC_HDMI_scl_o;
  wire IIC_HDMI_scl_t;
  wire IIC_HDMI_sda_i;
  wire IIC_HDMI_sda_io;
  wire IIC_HDMI_sda_o;
  wire IIC_HDMI_sda_t;
  wire aud_adcdat;
  wire aud_adclrc;
  wire [0:0]aud_bclk;
  wire aud_dacdat;
  wire aud_daclrc;
  wire aud_mclk;
  wire [0:0]gpio0_tri_i_0;
  wire [1:1]gpio0_tri_i_1;
  wire [2:2]gpio0_tri_i_2;
  wire [3:3]gpio0_tri_i_3;
  wire [4:4]gpio0_tri_i_4;
  wire [5:5]gpio0_tri_i_5;
  wire [6:6]gpio0_tri_i_6;
  wire [7:7]gpio0_tri_i_7;
  wire [8:8]gpio0_tri_i_8;
  wire [9:9]gpio0_tri_i_9;
  wire [0:0]gpio0_tri_io_0;
  wire [1:1]gpio0_tri_io_1;
  wire [2:2]gpio0_tri_io_2;
  wire [3:3]gpio0_tri_io_3;
  wire [4:4]gpio0_tri_io_4;
  wire [5:5]gpio0_tri_io_5;
  wire [6:6]gpio0_tri_io_6;
  wire [7:7]gpio0_tri_io_7;
  wire [8:8]gpio0_tri_io_8;
  wire [9:9]gpio0_tri_io_9;
  wire [0:0]gpio0_tri_o_0;
  wire [1:1]gpio0_tri_o_1;
  wire [2:2]gpio0_tri_o_2;
  wire [3:3]gpio0_tri_o_3;
  wire [4:4]gpio0_tri_o_4;
  wire [5:5]gpio0_tri_o_5;
  wire [6:6]gpio0_tri_o_6;
  wire [7:7]gpio0_tri_o_7;
  wire [8:8]gpio0_tri_o_8;
  wire [9:9]gpio0_tri_o_9;
  wire [0:0]gpio0_tri_t_0;
  wire [1:1]gpio0_tri_t_1;
  wire [2:2]gpio0_tri_t_2;
  wire [3:3]gpio0_tri_t_3;
  wire [4:4]gpio0_tri_t_4;
  wire [5:5]gpio0_tri_t_5;
  wire [6:6]gpio0_tri_t_6;
  wire [7:7]gpio0_tri_t_7;
  wire [8:8]gpio0_tri_t_8;
  wire [9:9]gpio0_tri_t_9;
  wire [2:0]pl_led_tri_o;
  wire tmds_tmds_clk_n;
  wire tmds_tmds_clk_p;
  wire [2:0]tmds_tmds_data_n;
  wire [2:0]tmds_tmds_data_p;

  IOBUF IIC_BUS_scl_iobuf
       (.I(IIC_BUS_scl_o),
        .IO(IIC_BUS_scl_io),
        .O(IIC_BUS_scl_i),
        .T(IIC_BUS_scl_t));
  IOBUF IIC_BUS_sda_iobuf
       (.I(IIC_BUS_sda_o),
        .IO(IIC_BUS_sda_io),
        .O(IIC_BUS_sda_i),
        .T(IIC_BUS_sda_t));
  IOBUF IIC_CAM_scl_iobuf
       (.I(IIC_CAM_scl_o),
        .IO(IIC_CAM_scl_io),
        .O(IIC_CAM_scl_i),
        .T(IIC_CAM_scl_t));
  IOBUF IIC_CAM_sda_iobuf
       (.I(IIC_CAM_sda_o),
        .IO(IIC_CAM_sda_io),
        .O(IIC_CAM_sda_i),
        .T(IIC_CAM_sda_t));
  IOBUF IIC_HDMI_scl_iobuf
       (.I(IIC_HDMI_scl_o),
        .IO(IIC_HDMI_scl_io),
        .O(IIC_HDMI_scl_i),
        .T(IIC_HDMI_scl_t));
  IOBUF IIC_HDMI_sda_iobuf
       (.I(IIC_HDMI_sda_o),
        .IO(IIC_HDMI_sda_io),
        .O(IIC_HDMI_sda_i),
        .T(IIC_HDMI_sda_t));
  design_1 design_1_i
       (.DDR_addr(DDR_addr),
        .DDR_ba(DDR_ba),
        .DDR_cas_n(DDR_cas_n),
        .DDR_ck_n(DDR_ck_n),
        .DDR_ck_p(DDR_ck_p),
        .DDR_cke(DDR_cke),
        .DDR_cs_n(DDR_cs_n),
        .DDR_dm(DDR_dm),
        .DDR_dq(DDR_dq),
        .DDR_dqs_n(DDR_dqs_n),
        .DDR_dqs_p(DDR_dqs_p),
        .DDR_odt(DDR_odt),
        .DDR_ras_n(DDR_ras_n),
        .DDR_reset_n(DDR_reset_n),
        .DDR_we_n(DDR_we_n),
        .FIXED_IO_ddr_vrn(FIXED_IO_ddr_vrn),
        .FIXED_IO_ddr_vrp(FIXED_IO_ddr_vrp),
        .FIXED_IO_mio(FIXED_IO_mio),
        .FIXED_IO_ps_clk(FIXED_IO_ps_clk),
        .FIXED_IO_ps_porb(FIXED_IO_ps_porb),
        .FIXED_IO_ps_srstb(FIXED_IO_ps_srstb),
        .IIC_BUS_scl_i(IIC_BUS_scl_i),
        .IIC_BUS_scl_o(IIC_BUS_scl_o),
        .IIC_BUS_scl_t(IIC_BUS_scl_t),
        .IIC_BUS_sda_i(IIC_BUS_sda_i),
        .IIC_BUS_sda_o(IIC_BUS_sda_o),
        .IIC_BUS_sda_t(IIC_BUS_sda_t),
        .IIC_CAM_scl_i(IIC_CAM_scl_i),
        .IIC_CAM_scl_o(IIC_CAM_scl_o),
        .IIC_CAM_scl_t(IIC_CAM_scl_t),
        .IIC_CAM_sda_i(IIC_CAM_sda_i),
        .IIC_CAM_sda_o(IIC_CAM_sda_o),
        .IIC_CAM_sda_t(IIC_CAM_sda_t),
        .IIC_HDMI_scl_i(IIC_HDMI_scl_i),
        .IIC_HDMI_scl_o(IIC_HDMI_scl_o),
        .IIC_HDMI_scl_t(IIC_HDMI_scl_t),
        .IIC_HDMI_sda_i(IIC_HDMI_sda_i),
        .IIC_HDMI_sda_o(IIC_HDMI_sda_o),
        .IIC_HDMI_sda_t(IIC_HDMI_sda_t),
        .aud_adcdat(aud_adcdat),
        .aud_adclrc(aud_adclrc),
        .aud_bclk(aud_bclk),
        .aud_dacdat(aud_dacdat),
        .aud_daclrc(aud_daclrc),
        .aud_mclk(aud_mclk),
        .gpio0_tri_i({gpio0_tri_i_9,gpio0_tri_i_8,gpio0_tri_i_7,gpio0_tri_i_6,gpio0_tri_i_5,gpio0_tri_i_4,gpio0_tri_i_3,gpio0_tri_i_2,gpio0_tri_i_1,gpio0_tri_i_0}),
        .gpio0_tri_o({gpio0_tri_o_9,gpio0_tri_o_8,gpio0_tri_o_7,gpio0_tri_o_6,gpio0_tri_o_5,gpio0_tri_o_4,gpio0_tri_o_3,gpio0_tri_o_2,gpio0_tri_o_1,gpio0_tri_o_0}),
        .gpio0_tri_t({gpio0_tri_t_9,gpio0_tri_t_8,gpio0_tri_t_7,gpio0_tri_t_6,gpio0_tri_t_5,gpio0_tri_t_4,gpio0_tri_t_3,gpio0_tri_t_2,gpio0_tri_t_1,gpio0_tri_t_0}),
        .pl_led_tri_o(pl_led_tri_o),
        .tmds_tmds_clk_n(tmds_tmds_clk_n),
        .tmds_tmds_clk_p(tmds_tmds_clk_p),
        .tmds_tmds_data_n(tmds_tmds_data_n),
        .tmds_tmds_data_p(tmds_tmds_data_p));
  IOBUF gpio0_tri_iobuf_0
       (.I(gpio0_tri_o_0),
        .IO(gpio0_tri_io[0]),
        .O(gpio0_tri_i_0),
        .T(gpio0_tri_t_0));
  IOBUF gpio0_tri_iobuf_1
       (.I(gpio0_tri_o_1),
        .IO(gpio0_tri_io[1]),
        .O(gpio0_tri_i_1),
        .T(gpio0_tri_t_1));
  IOBUF gpio0_tri_iobuf_2
       (.I(gpio0_tri_o_2),
        .IO(gpio0_tri_io[2]),
        .O(gpio0_tri_i_2),
        .T(gpio0_tri_t_2));
  IOBUF gpio0_tri_iobuf_3
       (.I(gpio0_tri_o_3),
        .IO(gpio0_tri_io[3]),
        .O(gpio0_tri_i_3),
        .T(gpio0_tri_t_3));
  IOBUF gpio0_tri_iobuf_4
       (.I(gpio0_tri_o_4),
        .IO(gpio0_tri_io[4]),
        .O(gpio0_tri_i_4),
        .T(gpio0_tri_t_4));
  IOBUF gpio0_tri_iobuf_5
       (.I(gpio0_tri_o_5),
        .IO(gpio0_tri_io[5]),
        .O(gpio0_tri_i_5),
        .T(gpio0_tri_t_5));
  IOBUF gpio0_tri_iobuf_6
       (.I(gpio0_tri_o_6),
        .IO(gpio0_tri_io[6]),
        .O(gpio0_tri_i_6),
        .T(gpio0_tri_t_6));
  IOBUF gpio0_tri_iobuf_7
       (.I(gpio0_tri_o_7),
        .IO(gpio0_tri_io[7]),
        .O(gpio0_tri_i_7),
        .T(gpio0_tri_t_7));
  IOBUF gpio0_tri_iobuf_8
       (.I(gpio0_tri_o_8),
        .IO(gpio0_tri_io[8]),
        .O(gpio0_tri_i_8),
        .T(gpio0_tri_t_8));
  IOBUF gpio0_tri_iobuf_9
       (.I(gpio0_tri_o_9),
        .IO(gpio0_tri_io[9]),
        .O(gpio0_tri_i_9),
        .T(gpio0_tri_t_9));
endmodule
