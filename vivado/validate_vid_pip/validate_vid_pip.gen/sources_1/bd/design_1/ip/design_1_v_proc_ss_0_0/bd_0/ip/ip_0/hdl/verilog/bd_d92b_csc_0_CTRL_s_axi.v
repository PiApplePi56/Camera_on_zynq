...
#(parameter
    ...
    output wire [11:0]                   ROffset,
    output wire [11:0]                   GOffset,
    output wire [11:0]                   BOffset,
    output wire [9:0]                    ClampMin,
    output wire [9:0]                    ClipMax,
...
//        bit 11~0 - ROffset[11:0] (Read/Write)
//        others   - reserved
// 0x9c : reserved
// 0xa0 : Data signal of GOffset
//        bit 11~0 - GOffset[11:0] (Read/Write)
//        others   - reserved
// 0xa4 : reserved
// 0xa8 : Data signal of BOffset
//        bit 11~0 - BOffset[11:0] (Read/Write)
//        others   - reserved
// 0xac : reserved
// 0xb0 : Data signal of ClampMin
//        bit 9~0 - ClampMin[9:0] (Read/Write)
//        others  - reserved
// 0xb4 : reserved
// 0xb8 : Data signal of ClipMax
//        bit 9~0 - ClipMax[9:0] (Read/Write)
//        others  - reserved
// 0xbc : reserved
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_K32 = 'b0;
    reg  [15:0]                   int_K33 = 'b0;
    reg  [11:0]                   int_ROffset = 'b0;
    reg  [11:0]                   int_GOffset = 'b0;
    reg  [11:0]                   int_BOffset = 'b0;
    reg  [9:0]                    int_ClampMin = 'b0;
    reg  [9:0]                    int_ClipMax = 'b0;
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ROFFSET_DATA_0: begin
                    rdata <= int_ROffset[11:0];
                end
                ADDR_GOFFSET_DATA_0: begin
                    rdata <= int_GOffset[11:0];
                end
                ADDR_BOFFSET_DATA_0: begin
                    rdata <= int_BOffset[11:0];
                end
                ADDR_CLAMPMIN_DATA_0: begin
                    rdata <= int_ClampMin[9:0];
                end
                ADDR_CLIPMAX_DATA_0: begin
                    rdata <= int_ClipMax[9:0];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ROFFSET_DATA_0)
            int_ROffset[11:0] <= (WDATA[31:0] & wmask) | (int_ROffset[11:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_GOFFSET_DATA_0)
            int_GOffset[11:0] <= (WDATA[31:0] & wmask) | (int_GOffset[11:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BOFFSET_DATA_0)
            int_BOffset[11:0] <= (WDATA[31:0] & wmask) | (int_BOffset[11:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CLAMPMIN_DATA_0)
            int_ClampMin[9:0] <= (WDATA[31:0] & wmask) | (int_ClampMin[9:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CLIPMAX_DATA_0)
            int_ClipMax[9:0] <= (WDATA[31:0] & wmask) | (int_ClipMax[9:0] & ~wmask);
    end
end
